...
function getEditRequest(previousOrNext) {
    try {
        ...
        const response = makeRequest("GET", encodeURIComponent(`/prescribe/edit?prescription_id=${prescriptionId}`))
        resetPageData("sign")
...
function getPayloads() {
    ...
    const payloads = filePayloads.concat(textPayloads).filter(Boolean).map(payload => JSON.parse(payload))
